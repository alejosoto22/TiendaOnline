...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        $sql = $con->prepare(
            'SELECT count(id) FROM productos WHERE id=? AND activo=1'
        );
        $sql->execute([$id]);
        if ($sql->fetchColumn() > 0) {
            ...
            while (($archivo = $dir->read()) != false) {
                if (
                    $archivo != 'principal.jpg' &&
                    (strpos($archivo, 'jpg') || strpos($archivo, 'jpeg'))
                ) {
  ...
  <div class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container">
      <a href="#" class="navbar-brand">
        <strong>Tienda Online Caldas Motor S.A.</strong>
      </a>
